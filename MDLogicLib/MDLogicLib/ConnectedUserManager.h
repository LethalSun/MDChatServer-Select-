...
namespace MDLogicLib
{
	...
	public:
		...
		void Init(const int maxSessionCount, TcpNet* pNetwork, MDServerNetLib::ServerConfig* pConfig, ILog* pLogger)
		{
		...
		void LoginCheck()
		{
			...
			// 60밀리초 마다 검사 전체 유저를 나눠서 검사한다. 일정 순간에 계산량이 급증하는 것을 막기위해
			...
			for (; m_LatestLogincheckIndex < lastCheckIndex; ++m_LatestLogincheckIndex)
			{
				...
				if (diff >= 180)
				{
					m_pRefLogger->Write(MDServerNetLib::LOG_TYPE::L_WARN, "%s | Login Wait Time Over. sessionIndex(%d).", __FUNCTION__, i);
					m_pRefNetwork->ForcedClose(i);
				}
			}
		}
