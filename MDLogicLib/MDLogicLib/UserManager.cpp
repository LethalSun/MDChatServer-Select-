...
{
	void MDLogicLib::UserManager::Init(const int maxUserCount)
	{
	...
	User * MDLogicLib::UserManager::AllocUserObjPoolIndex()
	{
	...
	void MDLogicLib::UserManager::ReleaseUserObjPoolIndex(const int index)
	{
		m_UserObjPoolIndex.push_back(index);
		m_UserObjPool[index].Clear();
	}

	ERROR_CODE MDLogicLib::UserManager::AddUser(const int sessionIndex, const char * pszID)
	{
	...
	ERROR_CODE MDLogicLib::UserManager::RemoveUser(const int sessionIndex)
	{
	...
	std::tuple<ERROR_CODE, User*> MDLogicLib::UserManager::GetUser(const int sessionIndex)
	{
		auto pUser = FindUser(sessionIndex);
	...
	User * MDLogicLib::UserManager::FindUser(const int sessionIndex)
	{
	...
	User * MDLogicLib::UserManager::FindUser(const char * pszID)
	{
